import astrotools.stats as astats
...
def geometricExposure(declination):
    ...
    xi = clip(xi, -1, 1)
    am = arccos(xi)
    exposure = cos(olat) * cos(declination) * sin(am) + am * sin(olat) * sin(declination)
...
def meanXmax(E, A, model='Epos 1.99'):
    ...
    lE = log10(E) - 1
    return X0 + D*lE + (xi - D/log(10) + delta*lE)*log(A)
...
def varianceXmax(E, A, model='Epos 1.99'):
    ...
    These are only the expected shower-to-shower fluctuations (eq. 2.8) for a single energy and mass number.
    See arXiv:1301.6637
    """
...
def xmaxDistribution(E, A, weights=None, model='Epos 1.99'):
    ...
    fE = (xi - D/log(10) + delta*lEcenter)
    s2p = p0 + p1*lEcenter + p2*(lEcenter**2)
    a = a0 + a1*lEcenter

    mlnA, vlnA = astats.binnedMeanAndVariance(log10(E)-1, log(A), lEbins, weights)
...
def lnADistribution(E, A, weights=None):
    ...
    return astats.binnedMeanAndVariance(log10(E), log(A), bins, weights)

def spectrum(E, weights=None):
    """
    Differential spectrum for given energies [EeV] and weights.
    """
    N, bins = histogram(log10(E) + 18, weights=weights, bins=25, range=(18.0, 20.5))
    Nerr = N**.5 # poisson error
    binWidths = 10**bins[1:] - 10**bins[:-1] # linear bin widths
    J = N / binWidths # make differential
#    logE = (bins[1:] + bins[:-1]) / 2 # logarithmic bin centers
...
fin = StringIO("""#logE, mean, stathi, statlo
...
dSpectrum = genfromtxt(fin, delimiter=',', names=True)

# Auger shower development data from arXiv:1107.4804 (ICRC11)
fin = StringIO("""#logElo, logEhi, logE, meanlogE, N, mean, mstat, msyshi, msyslo, std, sstat, ssyshi, ssyslo
...
dXmax = genfromtxt(fin, delimiter=',', names=True)

# Auger mass composition data, parsed from arXiv:1301.6637
# Epos 1.99 <lnA>
fin = StringIO("""#logE, syshi, syslo, mean, stathi, statlo
...
dmA1 = genfromtxt(fin, delimiter=',', names=True)
...
dvA1 = genfromtxt(fin, delimiter=',', names=True)
...
dmA2 = genfromtxt(fin, delimiter=',', names=True)
...
dvA2 = genfromtxt(fin, delimiter=',', names=True)
...
dmA3 = genfromtxt(fin, delimiter=',', names=True)
...
dvA3 = genfromtxt(fin, delimiter=',', names=True)
...
dmA4 = genfromtxt(fin, delimiter=',', names=True)
...
dvA4 = genfromtxt(fin, delimiter=',', names=True)
# --------------------- DATA -------------------------
...
def plotMeanLnA(xList=None, model='Epos 1.99'):
    """
    ...
    ax.set_xlim([18, 19.8])
    ax.set_ylim([0, 4])
    ax.set_yticks(arange(0, 4.5, 0.5))
    ax.set_yticklabels(['0', '', '1', '', '2', '', '3', '', '4'])
    return fig

def plotVarLnA(xList=None, model='Epos 1.99'):
    """
